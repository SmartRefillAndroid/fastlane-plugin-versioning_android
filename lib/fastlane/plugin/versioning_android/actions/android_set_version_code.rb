  ...
  module Actions
    ...
    class AndroidSetVersionCodeAction < Action
      def self.run(params)
        ...
        saved = Helper::VersioningAndroidHelper.save_key_to_gradle_file(gradle_file_path, "androidVersionCode", new_version_code)

        if saved == -1
          UI.user_error!("Unable to set the Version Code in gradle.properties file at #{gradle_file_path}.")
        end
