module Fastlane
  module Helper
    class VersioningAndroidHelper
      ...
      GRADLE_FILE_TEST = "/tmp/fastlane/tests/versioning/app/gradle.properties"
      ...
      def self.get_new_version_code(gradle_file, new_version_code)
        if new_version_code.nil?
          current_version_code = self.read_key_from_gradle_file(gradle_file, "androidVersionCode")
      ...
      def self.get_new_version_name(gradle_file, new_version_name, bump_type = nil)
        if new_version_name.nil?
          current_version_name = self.read_key_from_gradle_file(gradle_file, "version")
